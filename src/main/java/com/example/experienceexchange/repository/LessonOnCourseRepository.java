...
public class LessonOnCourseRepository extends HibernateAbstractDao<LessonOnCourse, Long> implements ILessonOnCourseRepository {
    ...
    private static final String JPQL_FIND_LESSONS_BY_USER = "SELECT ln FROM LessonOnCourse ln JOIN FETCH ln.course cr JOIN cr.usersInCourse us WHERE us.id =: userId ORDER BY ln.startLesson ASC";
    private static final String JPQL_FIND_LESSONS_BY_USER_BY_COURSE = "SELECT ln FROM LessonOnCourse ln JOIN FETCH ln.course cr JOIN cr.usersInCourse us WHERE cr.id=:courseId AND us.id =: userId ORDER BY ln.startLesson ASC";
